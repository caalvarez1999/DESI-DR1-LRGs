# Vendored from the author's personal library
# (MYLIBS/TAYLOR_emcee_tz_widepriors.py), verbatim -- the wide-priors
# variant of aux/taylor_tz.py. Note it isn't just wider priors: its C3
# Taylor coefficient also differs (`- 1/6*jz0` here vs. `- 1/6*jz0**2` in
# the narrow variant) -- an inconsistency present in the original library,
# kept as-is for behavioral fidelity rather than "fixed" here.
...
def lat(orden, grupo, z, z0, ages_dict, Hz0, qz0, jz0):
    ...
    C3 = (1.0 + qz0 + 1.0/2.0*qz0**2.0 - 1.0/6.0 * jz0) if orden >= 3 else 0
