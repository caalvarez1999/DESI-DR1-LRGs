...
# Vendored from the author's personal library (STACKING/MCMC3_burnin_maker.py),
# trimmed to just burnin() (the full t/Z/afe-fit variant -- what
# 9_FULLTMJ.py/pipeline/20_SPSfitTMJ.py use) and the autocorrelation helpers
# it depends on. Left out: burnin_metal (a Z/afe-only fit variant, not used
# by the TMJ pipeline stage) and onlyparanames (unused standalone helper).
# Also dropped: a dead `from getdist import loadMCSamples, plots` (imported,
# never used) and the module-level `import matplotlib.pyplot`, moved inline
# to the one `seplotea` branch that actually uses it.
...
def burnin(namefile, txt = 'no', what1="read_h5", what2="generate_burnin", seplotea=False, extrafiles = False, nwalkers = 50):
    ...
    if what2=="generate_burnin":
        ...
        if extrafiles:
            f=open("%s.paramnames"%(name_file_burnin),"w+")
            f.write("t t\n")
            f.write("Z [Z/H]\n")
            f.write("alpha "+"["+"\\alpha/Fe]")
            f.close()

            f=open("%s.ranges"%(name_file_burnin),"w")
            f.write("t "+str(0.1)+" "+str(15)+"\n")
            f.write("Z "+str(-2.25)+" "+str(0.67)+"\n")
            f.write("alpha "+str(-0.3)+" "+str(0.5)+"\n")
            f.close()
